...
#define DATA_READING_STATE		1

#define DATA_PROCESSING_STATE	2
...
static void MPU6050_Cal_Accel_Scale();
static void MPU6050_Cal_Gyro_Scale();
static void MPU6050_Cal_ComplemtaryFilter_Angels(MPU6050_t *Data_Struct);
static void MPU6050_Cal_Gyro_Angels(MPU6050_t *Data_Struct);

static void MPU6050_Calibrate();

static void MPU6050_Cal_Accel_Angels();
...
extern void MPU6050_Read_All()
{
	...
	int16_t temp;
	uint8_t counter=0;
	uint8_t state =DATA_READING_STATE ;
	...
	while(1){
		...
		switch(state){
			...
			case DATA_READING_STATE:
				...
				{
					counter=0;
					state=DATA_PROCESSING_STATE ;
				}

				break;

			case DATA_PROCESSING_STATE:

					printf("Data processing state \n");
					...
					MY_MPU.Gyro_X_RAW=(int16_t)((GYRO_BUF[0][0]+GYRO_BUF[0][1]+GYRO_BUF[0][2])/3.0);
					MY_MPU.Gyro_Y_RAW=(int16_t)((GYRO_BUF[1][0]+GYRO_BUF[1][1]+GYRO_BUF[1][2])/3.0);
					MY_MPU.Gyro_Z_RAW=(int16_t)((GYRO_BUF[2][0]+GYRO_BUF[2][1]+GYRO_BUF[2][2])/3.0);

					/*Data processing */

					MPU6050_Cal_Accel_Scale(&MY_MPU);

					MPU6050_Cal_Gyro_Scale(&MY_MPU);

					MPU6050_Cal_Accel_Angels(&MY_MPU);

					MPU6050_Cal_Gyro_Angels(&MY_MPU);

					MPU6050_Cal_ComplemtaryFilter_Angels(&MY_MPU);
...
extern void Accident_Detection()
{
	...
	while(1)
	{
		...
		if(		  (MY_MPU.ComFilter_Angle_X>=MAX_FLIPPING_Angle)||(MY_MPU.ComFilter_Angle_X<=MIN_FLIPPING_Angle)
				...
				||(MY_MPU.ComFilter_Angle_Z>=MAX_FLIPPING_Angle)||(MY_MPU.ComFilter_Angle_Z<=MIN_FLIPPING_Angle))
		{
			STM_EVAL_LEDOn(LED3);
...
static void MPU6050_Cal_Accel_Scale(MPU6050_t *Data_Struct)
{
...
static void MPU6050_Cal_Accel_Angels(MPU6050_t *Data_Struct)
{
	//char buf[20];
	//Roll angle
...
static void MPU6050_Cal_Gyro_Scale(MPU6050_t *Data_Struct)
{
	//char buf[20];
	//Accel Scale data
...
extern  float  Get_Angle_X(void){
	return MY_MPU.ComFilter_Angle_X;
}

extern  float  Get_Angle_Y(void){
	return MY_MPU.ComFilter_Angle_Y;
}


extern  float  Get_Angle_Z(void){
	return MY_MPU.ComFilter_Angle_Z;
}
...
extern void HAL_I2C_EVENT_CALLBack()
{
